...
def Decoder(inp):
    ...
    up2 = keras.layers.concatenate([up2, inp[2], inp[5]], axis=3, name = "merge_2")
    ...
    up3 = keras.layers.concatenate([up3, inp[1], inp[4]], axis=3, name = "merge_3")
    ...
    decoded = Conv2D(3, (3, 3), activation='sigmoid', padding='same', name = "EnOutimg")(Upconv3_2)
    convnet = Model(input = inp, output =  decoded, name="outimg")
    convnet.summary()
    return convnet
...
#%%
def classifier_network(input_img):
    Econv1_1 = Conv2D(16, (3, 3), activation='relu', padding='same', name = "clblock1_conv1")(input_img)
    Econv1_1 = BatchNormalization()(Econv1_1)
    Econv1_2 = Conv2D(16, (3, 3), activation='relu', padding='same',  name = "clblock1_conv2")(Econv1_1)
    Econv1_2 = BatchNormalization()(Econv1_2)
    pool1 = MaxPooling2D(pool_size=(2, 2),strides=(2,2),padding='same', name = "clblock1_pool1")(Econv1_2)
    ##
    Econv2_1 = Conv2D(64, (3, 3), activation='relu', padding='same', name = "clblock2_conv1")(pool1)
    Econv2_1 = BatchNormalization()(Econv2_1)
    Econv2_2 = Conv2D(64, (3, 3), activation='relu', padding='same', name = "clblock2_conv2")(Econv2_1)
    Econv2_2 = BatchNormalization()(Econv2_2)
    pool2= MaxPooling2D(pool_size=(2, 2),strides=(2,2), padding='same', name = "clblock2_pool1")(Econv2_2)
    ##
    flat_1 = Flatten(name="cl_flatten_layer")(pool2)
    dense_1 = Dense(units=1, activation='sigmoid', name="ClDense1")(flat_1)
    # dense_2 = Dense(units=1, activation='sigmoid', name="ClDense2")(dense_1)
    ##
    clnetwork = Model(input = input_img, output = [dense_1, Econv1_2, Econv2_2], name="class")
    ##
    clnetwork.summary()
    return clnetwork
...
conv4_l = Input(shape = (x_shape,y_shape,16))
conv5_l = Input(shape = (x_shape//(2**1),y_shape//(2**1),64))
decoded = Decoder( [HG_, conv1_l, conv2_l, conv3_l, conv4_l, conv5_l])
...
# claasifier
clnet = classifier_network(input_img)


# Combined
output_img = decoded([nck(encoded(input_img)[0]),
    encoded(input_img)[1],
    encoded(input_img)[2],
    encoded(input_img)[3],
    clnet(input_img)[1],
    clnet(input_img)[2]])
clres = clnet(input_img)[0]



#%%
model= Model(input = input_img, output = [output_img, clres] )
model.summary()

model.compile(loss={"outimg":'mean_squared_error',"class":'sparse_categorical_crossentropy'}, optimizer='rmsprop', metrics=['accuracy'])
...
def generator(frame_path, batchsize):
    ...
    while True:
        for i in range(0,len(filetuples),batchsize):
            ...
            classlist = np.array(classlist, dtype=float)
            yield inplist, {"outimg":outlist, "class":classlist}
...
for epoch in range(2):
    ...
    for frame_path in inpdirs:
        ...
        history = model.fit_generator(generator(frame_path, batchsize), steps_per_epoch=int(10000/batchsize), epochs=1,
            ...
            validation_steps=1)
        model.save('model_tomNjerry_twohead.h5')
