...
def Decoder(inp):
    up1 = keras.layers.concatenate([inp[0],inp[4]],  axis=3, name = "merge_11")
    up1 = Conv2DTranspose(128,(3,3),strides = (2,2), activation = 'relu', padding = 'same', name = "upsample_1")(up1)
    up1 = BatchNormalization()(up1)
    up1 = keras.layers.concatenate([up1, inp[3],inp[7]],  axis=3, name = "merge_1")
    ...
    up2 = keras.layers.concatenate([up2, inp[2],inp[6]], axis=3, name = "merge_2")
    ...
    up3 = keras.layers.concatenate([up3, inp[1],inp[5]], axis=3, name = "merge_3")
...
single_img = Input(shape=(x_shape,y_shape,1))
...
conv1_l_1 = Input(shape = (x_shape,y_shape,16))
conv2_l_1 = Input(shape = (x_shape//(2**1),y_shape//(2**1),64))
conv3_l_1 = Input(shape = (x_shape//(2**2),y_shape//(2**2),128))
decoded = Decoder( [HG_, conv1_l, conv2_l, conv3_l,HG_1, conv1_l_1, conv2_l_1, conv3_l_1])
...
#%%
# Combined
output_img = decoded([nck(encoded(input_img)[0]), encoded(input_img)[1], encoded(input_img)[2], encoded(input_img)[3],nck(encoded_1(single_img)[0]), encoded_1(single_img)[1], encoded_1(single_img)[2], encoded_1(single_img)[3]])
model= Model(input = [single_img,input_img], output = output_img )
model.summary()
    ...
    while True:
        for i in range(0,len(filetuples),batchsize):
            ...
            for f2, f1, f0 in ft:
                ...
                singlelist.append(im0g.reshape(360, 480, 1))
                outlist.append(im0)
            inplist, outlist,singlelist = np.array(inplist)/255.0, np.array(outlist)/255.0,np.array(singlelist)/255.0
            yield {'input_1':inplist,'input_2':singlelist}, outlist
            

# it = generator('./ColorFrames4',1)
# am = next(it)
# print(am)
batchsize = 8
...
for epoch in range(3):
    ...
    for frame_path in inpdirs:
        ...
        history = model.fit_generator(generator(frame_path, batchsize), steps_per_epoch=int(9000/batchsize), epochs=1,
            ...
            validation_steps=1)
        model.save('model_tomNjerry_feedback.h5')
